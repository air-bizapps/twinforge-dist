...
# tests/key-parity-test.sh is what asserts the two agree.
#
...
# design -- a separate `sign` job, in an Environment with a required reviewer,
# whose only secret is the private key -- and no key material belongs in this
# public repository until then. A release that publishes a manifest this script
# would accept is *supposed* to fail today: failing closed is the whole design,
# and an "accept anything while we get set up" phase is exactly the hole that
# someone who can write this repository would use.
